...
async function getStock(stock) {
  const response = await fetch(`https://stock-price-checker-proxy.freecodecamp.rocks/v1/stock/${stock}/quote`); //fetch stock symbol
  const { symbol, latestPrice } = await response.json(); //grab symbol and latestPrice
  ...
  app.route('/api/stock-prices')
    .get(async function (req, res){
      ...
      if(!symbol) {
        res.json({ stockData: { likes: like ? 1: 0 } }); //if not found, return likes
        return
      }
